...
import {
  MOUNT_CATEGORY_OPTIONS,
  MOUNT_CATEGORY_ORDER,
  getCategoryTheme,
  getMountEmoji,
  RARITY_OPTIONS,
  isHuntRarity,
...
function MountFormModal({ open, onClose, item, onSaved }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        toast.success('Montaria atualizada com sucesso.');
      } else {
        await itemsService.create(payload);
        toast.success('Montaria cadastrada com sucesso.');
      }
      onSaved();
      onClose();
    } catch (err) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEditing ? 'Editar Montaria' : 'Nova Montaria'}
      size="lg"
    >
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="mount-title" className="mb-1 block text-sm text-zinc-300">Nome da Montaria</label>
          <input
            id="mount-title"
            name="title"
            required
            ...
            placeholder="Gillian's Steed"
          />
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="mount-category" className="mb-1 block text-sm text-zinc-300">Tipo de desbloqueio</label>
            <select id="mount-category" name="category" value={form.category} onChange={handleChange} className="input-field">
              {MOUNT_CATEGORY_OPTIONS.map((cat) => (
          ...
          <div>
            <label htmlFor="mount-rarity" className="mb-1 block text-sm text-zinc-300">Selo de Raridade</label>
            <select id="mount-rarity" name="rarity" value={form.rarity} onChange={handleChange} className="input-field">
        ...
        <div>
          <label htmlFor="mount-status" className="mb-1 block text-sm text-zinc-300">Status</label>
          <select id="mount-status" name="status" value={form.status} onChange={handleChange} className="input-field">
        ...
        <div>
          <label className="mb-2 block text-sm text-zinc-300">Dificuldade de obtenção (1–5)</label>
          <StarRating
            value={form.rating}
            interactive
            size="lg"
        ...
        <div>
          <label htmlFor="mount-description" className="mb-1 block text-sm text-zinc-300">Descrição</label>
          <textarea
            ...
            placeholder="Montaria base desbloqueada no Ato 3..."
          />
        </div>

        <div>
          <label htmlFor="mount-howTo" className="mb-1 block text-sm text-zinc-300">Como desbloquear</label>
          <textarea
            id="mount-howTo"
            name="howTo"
            rows={3}
            ...
            onChange={handleChange}
            className="input-field resize-none"
            placeholder="Passo a passo para obter a montaria..."
          />
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="mount-averageTime" className="mb-1 block text-sm text-zinc-300">Tempo médio</label>
            <input
              id="mount-averageTime"
              name="averageTime"
          ...
          <div>
            <label htmlFor="mount-location" className="mb-1 block text-sm text-zinc-300">Localização</label>
            <input
              id="mount-location"
              name="location"
              value={form.location}
              onChange={handleChange}
              className="input-field"
              placeholder="Stable Master — Scosglen"
            />
          </div>
        </div>

        <div>
          <label htmlFor="mount-guide" className="mb-1 block text-sm text-zinc-300">Guia completo (detalhes extras)</label>
          <textarea
            id="mount-guide"
            name="guide"
            rows={8}
            ...
            placeholder="Dicas, requisitos de Renown, custos..."
          />
        </div>
...
export function MountsPage() {
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(`Excluir a montaria "${item.title}"?`)) return;

    setDeletingId(item.id);
    try {
      await itemsService.remove(item.id);
      toast.success('Montaria removida.');
      await loadItems();
    } catch (err) {
      toast.error(getErrorMessage(err));
    } finally {
  ...
  const renderGrid = (gridItems) => (
    gridItems.map((item) => (
      <ItemCard
        key={item.id}
        item={item}
        getEmoji={getMountEmoji}
        clickHint="🧭 Clique para ver o guia da montaria"
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs uppercase tracking-widest text-red-600">🏇 Estábulos de Sanctuary</p>
          <h1 className="font-display text-2xl font-bold text-zinc-100 sm:text-3xl">
            🐎 Guia de Montarias
          </h1>
          <p className="mt-1 text-sm text-zinc-400">
            {items.length} montarias no grimório — {huntStats.hunted}/{huntStats.total} raras desbloqueadas.
          </p>
        </div>
      ...
      <div className="mb-6">
        <p className="mb-2 text-xs uppercase tracking-widest text-zinc-500">📂 Tipo de desbloqueio</p>
        <div className="flex flex-wrap gap-2">
          ...
          {MOUNT_CATEGORY_OPTIONS.map((option) => (
            <button
              key={option.value}
              type="button"
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {[
          ...
          { value: 'CONCLUIDO', label: 'Desbloqueadas', emoji: '✅' },
      ...
      {loading ? (
        ...
        <div className="card py-24 text-center text-zinc-500">
          <span className="mb-3 block text-4xl" aria-hidden="true">🏇</span>
          Nenhuma montaria encontrada. Registre sua primeira montaria!
        </div>
      ) : groupedItems ? (
            ...
            <section key={category}>
              <div className="mb-4 flex items-center gap-3 border-b border-zinc-800 pb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">{groupItems.length} montarias</p>
                </div>
              </div>
